...
every `.github/actions/**/action.yml`, and every file under `scripts/` those
workflows or actions run. Each `aws <service> <verb>` in them becomes an IAM
...
def sources() -> list[Path]:
    ...
    files = [*WORKFLOWS, *sorted(ACTIONS_DIR.glob("**/action.yml"))]
    seen: list[Path] = []
    queue = list(files)
    while queue:
        path = queue.pop(0)
