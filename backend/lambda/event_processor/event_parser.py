...
def parse_kinesis_events(
    kinesis_event: Dict[str, Any], skip_errors: bool = False
    ...
    for record in records:
        try:
            ...
            try:
                ...
                error_msg = f"Failed to decode base64 data for sequence {sequence_number}: {e!s}"
            ...
            try:
                ...
                error_msg = (
                    f"Failed to decode UTF-8 for sequence {sequence_number}: {e!s}"
                )
                logger.error(error_msg)
                if skip_errors:
                    errors.append(
            ...
            try:
                ...
                error_msg = (
                    f"Failed to parse JSON for sequence {sequence_number}: {e!s}"
                )
                logger.error(error_msg)
                if skip_errors:
                    errors.append(
