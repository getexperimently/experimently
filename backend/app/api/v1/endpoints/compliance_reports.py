...
EDITION: Enterprise. This module holds the *bodies* of
...
The routes themselves stay declared on the Community compliance router
(``api/v1/endpoints/compliance.py``), which reaches these handlers through
``core/enterprise_features`` and answers HTTP 501 when this module is not
installed.

``GET /api/v1/compliance/audit-events`` is Community and lives entirely in
``compliance.py`` — it reads ``audit_events_v2`` through the Community
