...
class CustomJsonFormatter(JsonFormatter):
    ...
    def __init__(self):
        ...
        self._style = logging.PercentStyle(
            fmt
        )  # Initialize style before super().__init__()
        super().__init__(
            fmt=fmt,
...
def setup_logging(
    ...
    if enable_cloudwatch:
        try:
            ...
            stream_name = (
                f"{socket.gethostname()}-{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}"
            )
...
def get_logger(name: str) -> logging.Logger:
    ...
    try:
        ...
        # getLogger() failed (a broken logging config); fall back to a
        # standalone logger so callers still get something usable.
