...
class RuleValidator:
    ...
    def validate_targeting_rules(
        ...
        try:
            ...
            performance_warnings = self._analyze_performance_impact(
                targeting_rules, complexity_score
            )
    ...
    def _validate_schema_structure(
        ...
        duplicate_ids = {x for x in rule_ids if rule_ids.count(x) > 1}
        if duplicate_ids:
            issues.append(
                ValidationIssue(
                    severity=ValidationSeverity.ERROR,
        ...
        if not targeting_rules.version or not re.match(
            r"^\d+\.\d+$", targeting_rules.version
        ):
            issues.append(
                ValidationIssue(
                    severity=ValidationSeverity.WARNING,
    ...
    def _validate_targeting_rule(
        self, rule: TargetingRule
    ...
    def _validate_rule_group(
        ...
        for i, condition in enumerate(rule_group.conditions):
            ...
            condition_issues = self._validate_condition(
                condition, rule_id, condition_path
            )
        ...
        if (
            rule_group.operator == LogicalOperator.NOT
            and len(rule_group.conditions) + len(rule_group.groups or []) > 1
        ):
            issues.append(
                ValidationIssue(
                    severity=ValidationSeverity.WARNING,
    ...
    def _validate_condition(
        ...
        operator_issues = self._validate_operator_compatibility(
            condition, rule_id, path
        )
    ...
    def _validate_operator_compatibility(
        ...
        try:
            ...
            if (
                operator in [OperatorType.BETWEEN, OperatorType.GEO_DISTANCE]
                and additional_value is None
            ):
                issues.append(
                    ValidationIssue(
                        severity=ValidationSeverity.ERROR,
    ...
    def _check_condition_performance(
        ...
        # Large arrays
        if operator in [OperatorType.IN, OperatorType.NOT_IN] and isinstance(
            value, (list, tuple)
        ):
            if len(value) > 100:
                issues.append(
    ...
    def _check_rule_conflicts(
        ...
        if targeting_rules.default_rule and targeting_rules.rules:
            ...
            for rule in targeting_rules.rules:
                if rule.rollout_percentage == 100 and self._rule_catches_all_users(
                    rule
                ):
                    issues.append(
                        ValidationIssue(
    ...
    def _group_catches_all_users(self, group: RuleGroup) -> bool:
        ...
        if (
            ...
            and group.conditions[0].attribute in ["registered_user", "active_user"]
            and group.conditions[0].value is True
        ):
            return True
    ...
    def _analyze_performance_impact(
        ...
        if complexity_score > 50:
            warnings.append(
                f"High complexity score ({complexity_score}) may impact evaluation performance"
            )

        # Too many rules
        if len(targeting_rules.rules) > 20:
            warnings.append(
                f"Large number of rules ({len(targeting_rules.rules)}) may slow evaluation"
            )
        ...
        if max_depth > 5:
            warnings.append(
                f"Deep rule nesting (depth {max_depth}) may impact performance"
            )
        ...
        def calculate_group_depth(group: RuleGroup, current_depth: int = 0) -> int:
            ...
            if group.groups:
                for nested_group in group.groups:
                    nested_depth = calculate_group_depth(
                        nested_group, current_depth + 1
                    )
