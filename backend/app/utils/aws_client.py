...
class AWSClient:
    ...
    def init_cloudwatch_logs(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Successfully initialized CloudWatch Logs client in region {region}"
            )
            return True

        except Exception as e:
            logger.error(f"Failed to initialize CloudWatch Logs client: {e!s}")
            self.logs_client = None
            return False
    ...
    def init_cloudwatch_metrics(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Successfully initialized CloudWatch Metrics client in region {region}"
            )
            return True

        except Exception as e:
            logger.error(f"Failed to initialize CloudWatch Metrics client: {e!s}")
            self.metrics_client = None
            return False
    ...
    def send_metric(
        ...
        try:
            ...
            logger.error(f"Failed to put metric data: {e!s}")
            return False
