...
class Condition(BaseModel):
    ...
    additional_value: Optional[Any] = Field(
        None,
        description="Additional value for operators that need two values (e.g., between)",
    )
    attribute_type: Optional[AttributeType] = Field(
        None, description="Type of the attribute for validation"
    )
    validation_schema: Optional[Dict[str, Any]] = Field(
        None, description="JSON schema for validating the attribute value"
    )
    ...
    def validate_attribute_name(cls, v):
        ...
        if not v.replace("_", "").replace(".", "").isalnum():
            raise ValueError(
                "Attribute name must contain only alphanumeric characters, underscores, and dots"
            )
    ...
    def validate_operator_value_compatibility(self):
        ...
        if operator in [
            ...
            if not isinstance(value, (list, tuple, set)):
                raise ValueError(
                    f"Operator {operator} requires a collection (list/array) as value"
                )
        ...
        if operator == OperatorType.GEO_DISTANCE:
            if not isinstance(value, (list, tuple)) or len(value) != 2:
                raise ValueError(
                    "GEO_DISTANCE operator requires value as [latitude, longitude]"
                )
            try:
        ...
        if operator == OperatorType.JSON_PATH:
            if not isinstance(value, str) or not value.startswith("$."):
                raise ValueError(
                    "JSON_PATH operator requires value as JSONPath string starting with '$.'"
                )
        ...
        if operator == OperatorType.TIME_WINDOW:
            if (
                ...
                raise ValueError(
                    "TIME_WINDOW operator requires value as dict with 'start' and 'end' keys"
                )
...
class RuleGroup(BaseModel):
    ...
    operator: LogicalOperator = Field(
        LogicalOperator.AND, description="Logical operator to combine conditions"
    )
    conditions: List[Condition] = Field(
        default_factory=list, description="List of conditions in this group"
    )
    groups: Optional[List["RuleGroup"]] = Field(
        default_factory=list, description="Nested rule groups for complex rules"
    )
...
class TargetingRule(BaseModel):
    ...
    description: Optional[str] = Field(
        None, description="Description of the rule's purpose"
    )
    ...
    rollout_percentage: int = Field(
        ...
        ge=0,
        le=100,
        description="Percentage of users who match conditions to include",
    )
    priority: int = Field(
        0, description="Priority of the rule (lower number = higher priority)"
    )
...
class TargetingRules(BaseModel):
    ...
    rules: List[TargetingRule] = Field(
        default_factory=list, description="List of targeting rules"
    )
    default_rule: Optional[TargetingRule] = Field(
        None, description="Default rule if no others match"
    )
