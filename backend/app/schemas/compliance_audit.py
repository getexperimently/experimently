...
class ComplianceAuditEventCreate(BaseModel):
    ...
    request_id: Optional[str] = Field(
        None, max_length=128, description="Request identifier for tracing"
    )

    # Resource details
    resource_id: Optional[str] = Field(
        None, max_length=128, description="ID of the affected resource"
    )

    # Change data
    old_value: Optional[Dict[str, Any]] = Field(
        None, description="Previous state of the resource (JSON)"
    )
    new_value: Optional[Dict[str, Any]] = Field(
        None, description="New state of the resource (JSON)"
    )
...
class ComplianceAuditEventResponse(BaseModel):
    ...
    hmac_signature: Optional[str] = Field(
        None, description="HMAC-SHA256 signature for tamper detection"
    )
...
class ComplianceAuditEventListResponse(BaseModel):
    ...
    items: List[ComplianceAuditEventResponse] = Field(
