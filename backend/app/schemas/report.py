...
class ReportBase(BaseModel):
    ...
    report_type: str = Field(
        ...,
        description="Type of report (experiment_result, feature_flag_usage, user_activity, custom)",
    )
    is_public: bool = Field(
        False, description="Whether the report is publicly viewable"
    )
    data: Optional[Dict[str, Any]] = Field(
        None, description="Stored report data/results"
    )
    query_definition: Optional[Dict[str, Any]] = Field(
        None, description="Definition of data query"
    )
    visualization_config: Optional[Dict[str, Any]] = Field(
        None, description="Visualization settings"
    )
    experiment_id: Optional[UUID] = Field(None, description="Associated experiment ID")
    feature_flag_id: Optional[UUID] = Field(
        None, description="Associated feature flag ID"
    )
...
class ReportUpdate(BaseModel):
    ...
    is_public: Optional[bool] = Field(
        None, description="Whether the report is publicly viewable"
    )
    data: Optional[Dict[str, Any]] = Field(
        None, description="Stored report data/results"
    )
