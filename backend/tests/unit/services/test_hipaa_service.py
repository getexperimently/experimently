"""
Unit tests for EP-050: HIPAA Service (backend/app/services/hipaa_service.py).
...
Run with:
    ...
    python -m pytest backend/tests/unit/services/test_hipaa_service.py -v
"""
...
class TestLogPhiAccess:
    ...
    def test_creates_audit_log_record(self, svc, mock_db):
        ...
        with patch("backend.app.services.hipaa_service.PHIAuditLog") as MockLog:
            mock_instance = Mock()
    ...
    def test_retention_years_set_to_6(self, svc, mock_db):
        ...
        with patch("backend.app.services.hipaa_service.PHIAuditLog") as MockLog:
            ...
            with patch("backend.app.services.hipaa_service.settings", mock_settings):
                svc.log_phi_access(
                    db=mock_db,
                    user_id=uuid.uuid4(),
                    resource_type="experiment",
...
class TestCreateBaa:
    ...
    def test_creates_baa_and_commits(self, svc, mock_db):
        ...
        with patch("backend.app.services.hipaa_service.BAAConfig") as MockBAA:
            mock_instance = Mock()
...
class TestCheckDataResidency:
    ...
    def test_allowed_region_returns_true(self, svc):
        ...
        with patch("backend.app.services.hipaa_service.settings") as mock_settings:
...
class TestEncryptDecryptDelegation:
    ...
    def test_encrypt_delegates_to_phi_encryption(self, svc):
        ...
        with patch("backend.app.services.hipaa_service.PHIEncryption") as MockEnc:
