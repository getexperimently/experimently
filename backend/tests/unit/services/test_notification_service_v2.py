...
class TestNotifyExperimentStarted:
    ...
    def test_calls_slack_send(self):
        svc = make_service()
        with patch.object(svc._slack, "send_experiment_started", return_value=True) as mock_sl:
    ...
    def test_calls_email_send(self):
        svc = make_service()
        with patch.object(svc._email, "send_experiment_started_email", return_value=True) as mock_em:
            with patch.object(svc, "send_webhook", return_value=True):
                with patch.object(svc._slack, "send_experiment_started", return_value=True):
                    svc.notify_experiment_started("exp-id-1", "My Experiment", owner_email="o@example.com")
                    mock_em.assert_called_once()

    def test_returns_bool(self):
        svc = make_service()
        with patch.object(svc, "send_webhook", return_value=False):
            with patch.object(svc._slack, "send_experiment_started", return_value=False):
                with patch.object(svc._email, "send_experiment_started_email", return_value=False):
...
class TestNotifyExperimentEnded:
    def test_calls_slack(self):
        svc = make_service()
        with patch.object(svc._slack, "send_experiment_completed", return_value=True) as mock_sl:
            with patch.object(svc, "send_webhook", return_value=True):
                svc.notify_experiment_ended("exp-id-1", "My Experiment", winning_variant="Variant B")
                mock_sl.assert_called_once()

    def test_passes_winner_to_email(self):
        svc = make_service()
        with patch.object(svc._email, "send_experiment_completed_email", return_value=True) as mock_em:
            with patch.object(svc, "send_webhook", return_value=True):
                with patch.object(svc._slack, "send_experiment_completed", return_value=True):
                    svc.notify_experiment_ended("id", "Name", winning_variant="B", owner_email="x@y.com")
                    mock_em.assert_called_once()
...
class TestNotifySafetyRollback:
    def test_calls_slack_alert(self):
        svc = make_service()
        with patch.object(svc._slack, "send_safety_rollback_alert", return_value=True) as mock_sl:
            with patch.object(svc, "send_webhook", return_value=True):
    ...
    def test_calls_webhook(self):
        svc = make_service()
        with patch.object(svc, "_send", return_value=True) as mock_wh:
            with patch.object(svc._slack, "send_safety_rollback_alert", return_value=False):
                svc.notify_safety_rollback("flag-id", "my-flag", "latency exceeded")
                mock_wh.assert_called_once()

    def test_never_raises(self):
        svc = make_service()
        with patch.object(svc._slack, "send_safety_rollback_alert", side_effect=Exception("slack down")):
...
class TestNotifyRolloutAdvanced:
    def test_calls_slack(self):
        svc = make_service()
        with patch.object(svc._slack, "send_rollout_advanced", return_value=True) as mock_sl:
            with patch.object(svc, "send_webhook", return_value=True):
    ...
    def test_never_raises(self):
        svc = make_service()
        with patch.object(svc._slack, "send_rollout_advanced", side_effect=Exception("network")):
