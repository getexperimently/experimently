"""
Enterprise: ``AuditLogService.log()`` signs events (HMAC-SHA256, EP-033).

Split out of ``test_compliance_api.py``, which now tests only what a
Community build does.  These assertions hold because the Enterprise
registration installs ``AuditSigningService`` through ``hooks.audit_signer``;
under the Community ``NullAuditSigner`` the signature is ``None`` by design.
"""
