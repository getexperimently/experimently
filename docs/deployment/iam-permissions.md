...
Confirm the whole list with `aws iam simulate-principal-policy` before the
...
The dashboard rollout script (#69, `scripts/ecs_rolling_rollout.sh`) adds
`ecs:UpdateService` and `ecs:ListTasks`. Once a deploy runs it, a role without
them stops at the dashboard's `update-service` with "ecs:UpdateService was
denied", after the API has already shifted. Re-apply the policy before that
deploy as well.
