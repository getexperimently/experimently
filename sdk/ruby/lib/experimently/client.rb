...
module Experimently
  ...
  class Client
    ...
    def evaluate_flag(flag_key, user_id, _attributes = {})
      ...
      warn "[Experimently] evaluate_flag error: #{e.message}"
    ...
    def get_assignment(experiment_key, user_id, attributes = {})
      ...
      unless data.is_a?(Hash) && data['variant_name']
        warn "[Experimently] get_assignment: malformed response for #{experiment_key}"
        return nil
      end
      ...
      warn "[Experimently] get_assignment error: #{e.message}"
      nil
    end
    ...
    def track(event_name, user_id, properties: {}, experiment_key: nil, feature_flag_key: nil,
      ...
      warn "[Experimently] track error: #{e.message}"
      false
    end
    ...
    def track_batch(events)
      ...
      bodies.each_slice(BATCH_LIMIT) do |slice|
        begin
          ...
          warn "[Experimently] track_batch error: #{e.message}"
          failure += slice.size
