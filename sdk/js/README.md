...
const { enabled, config } = await client.evaluateFlag('new_search', user);
...
| `getAllFlags(userId)` | `Promise<Record<string, boolean>>` (not cached) | rejects |
...
```bash
npm install
npm test          # 95 Jest tests (fetch mocked)
