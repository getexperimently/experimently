/**
 * Pure-TypeScript MD5 (RFC 1321). No runtime dependencies, so the same code
 * runs in Node and in browsers without pulling in `crypto` polyfills.
 *
 * Output is byte-for-byte identical to Node's
 * `crypto.createHash('md5').update(input, 'utf8').digest()`; the unit tests
 * cross-check it against Node's implementation and the shared golden vectors.
 */
...
function FF(a: number, b: number, c: number, d: number, x: number, s: number, t: number): number {
  return safeAdd(rotl(safeAdd(safeAdd(a, F(b, c, d)), safeAdd(x, t)), s), b);
}
function GG(a: number, b: number, c: number, d: number, x: number, s: number, t: number): number {
  return safeAdd(rotl(safeAdd(safeAdd(a, G(b, c, d)), safeAdd(x, t)), s), b);
}
function HH(a: number, b: number, c: number, d: number, x: number, s: number, t: number): number {
  return safeAdd(rotl(safeAdd(safeAdd(a, H(b, c, d)), safeAdd(x, t)), s), b);
}
function II(a: number, b: number, c: number, d: number, x: number, s: number, t: number): number {
  return safeAdd(rotl(safeAdd(safeAdd(a, I(b, c, d)), safeAdd(x, t)), s), b);
}

/** UTF-8 encode a string. Uses TextEncoder when available, else a manual encoder. */
...
/** Pad to a multiple of 64 bytes and append the 64-bit little-endian bit length. */
...
/** MD5 digest of the UTF-8 encoding of `input` as 16 bytes. */
export function md5Bytes(input: string): Uint8Array {
  const data = pad(utf8Bytes(input));
