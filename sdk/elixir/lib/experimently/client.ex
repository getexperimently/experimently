defmodule Experimently.Client do
  @moduledoc """
  GenServer client for the Experimently public API.
  ...
  ## Usage
      ...
      {:ok, client} = Experimently.Client.start_link(
        base_url: "http://localhost:8000",
        api_key: "your-api-key"
      )

      {:ok, %Experimently.FlagEvaluation{enabled: enabled}} =
        Experimently.Client.evaluate_flag(client, "my-flag", "user-123")

      {:ok, %Experimently.Assignment{variant_name: variant}} =
        Experimently.Client.get_assignment(client, "experiment-key", "user-123")

      :ok = Experimently.Client.track(client, "purchase", "user-123", %{sku: "pro"},
              experiment_key: "experiment-key", value: 12.5)

      Experimently.Client.stop(client)
  """
  ...
  alias Experimently.{Assignment, BatchResult, Cache, Config, FlagEvaluation}
  ...
  def init(%Config{} = config) do
    ...
    state = %{
      config: config,
      cache: cache_pid,
      http: config.http_client || Experimently.HttpClient,
      inflight: MapSet.new()
    }
  ...
  def handle_cast({:track, event_name, user_id, properties, opts}, state) do
    requests =
      try do
        ...
        error ->
          Logger.warning(
            "Experimently: could not build track request for #{inspect(event_name)}: " <>
    ...
    {_pid, ref} =
      spawn_monitor(fn ->
        case http.post(config, path, body) do
          ...
          {:error, reason} ->
            Logger.warning(
              "Experimently: failed to track event #{inspect(event_name)} " <>
                "(#{path}): #{inspect(reason)}"
            )
        end
      end)
