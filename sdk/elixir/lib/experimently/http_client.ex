defmodule Experimently.HttpBehaviour do
  @moduledoc """
  ...
  @callback get(config :: Experimently.Config.t(), path :: String.t()) ::
              {:ok, map()} | {:error, term()}

  @callback post(
              config :: Experimently.Config.t(),
              path :: String.t(),
              body :: map()
            ) :: {:ok, map()} | {:error, term()}
end

defmodule Experimently.HttpClient do
  @moduledoc """
  ...
  alias Experimently.Config

  @user_agent ~c"Experimently-Elixir-SDK/0.1.0"
