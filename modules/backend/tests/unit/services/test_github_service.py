...
class TestGitHubServiceInit:
    def test_init_with_token(self):
        ...
        from modules.backend.app.services.integrations.github_service import (
            GitHubService,
        )
...
class TestGitHubWebhookSignatureHardening:
    """GitHub signs every delivery, so a signature is the only thing accepted.

    These cover what the endpoint now leans on: a service with no secret
    verifies nothing, and a header an attacker chooses cannot raise.
    """

    @staticmethod
    def _service(webhook_secret: str = "gh-webhook-secret"):
        from modules.backend.app.services.integrations.github_service import (
            GitHubService,
        )
    ...
    def test_empty_signature_header_is_rejected(self):
        """No header at all is a refusal, not a skipped check."""
        svc = self._service()
        assert svc.verify_webhook_signature(b'{"action": "opened"}', "") is False

    @pytest.mark.regression
    def test_non_ascii_signature_is_rejected_not_raised(self):
        """Starlette decodes headers as latin-1.

        `hmac.compare_digest` raises TypeError on a non-ASCII str, so a
        comparison on str would let any sender turn a header into a 500.
        """
        svc = self._service()
        assert svc.verify_webhook_signature(b"payload", "sha256=caf\xe9") is False
    ...
    def test_signature_of_another_body_is_rejected(self):
        ...
        signature = (
            "sha256="
            + hmac_lib.new(secret.encode(), b'{"a": 1}', hashlib.sha256).hexdigest()
        )
        svc = self._service(secret)
        assert svc.verify_webhook_signature(b'{"a": 2}', signature) is False

    def test_uppercase_hex_signature_is_rejected(self):
        """GitHub sends lowercase hex; a constant-time compare is exact."""
        secret = "gh-webhook-secret"
        body = b'{"action": "opened"}'
        signature = (
            "sha256="
            + hmac_lib.new(secret.encode(), body, hashlib.sha256).hexdigest().upper()
        )
        svc = self._service(secret)
        assert svc.verify_webhook_signature(body, signature) is False
