...
from modules.backend.app.api.v1.endpoints.realtime_counters import get_counter_service
...
class TestIncrementCounter:
    ...
    def test_developer_can_increment_counter(self, developer_client, counter_mock):
        """DEVELOPER holds experiment:update, so may write counters."""
        payload = {
            "experiment_id": _TEST_EXP_ID,
            "variant_id": _TEST_VARIANT_A,
            "counter_type": "event",
    ...
    def test_analyst_cannot_increment_counter(self, analyst_client, counter_mock):
        """ANALYST may read every experiment and change none — 403 here.

        Regression: the route used to require only an authenticated session,
        so a read-only account could POST conversions that BanditScheduler
        prefers over PostgreSQL and re-weight live traffic.
        """
        payload = {
            "experiment_id": _TEST_EXP_ID,
            "variant_id": _TEST_VARIANT_A,
        ...
        assert not counter_mock.increment_counter.called

    @pytest.mark.regression
    def test_viewer_cannot_increment_counter(self, viewer_client, counter_mock):
        """VIEWER is read-only everywhere, including the counters."""
        payload = {
            "experiment_id": _TEST_EXP_ID,
            "variant_id": _TEST_VARIANT_A,
            "counter_type": "conversion",
...
class TestBulkIncrement:
    ...
    def test_developer_can_bulk_increment(self, developer_client, counter_mock):
        """DEVELOPER holds experiment:update, so may bulk-write counters."""
        payload = {
            "increments": [
                {
                    "experiment_id": _TEST_EXP_ID,
    ...
    @pytest.mark.regression
    def test_analyst_cannot_bulk_increment(self, analyst_client, counter_mock):
        """Bulk is the same write as increment, and was open in the same way.

        Regression: 100 forged conversions in one request, from a read-only
        account.
        """
        payload = {
            "increments": [
    ...
    @pytest.mark.regression
    def test_viewer_cannot_bulk_increment(self, viewer_client, counter_mock):
        """VIEWER is read-only everywhere, including the counters."""
        payload = {
            "increments": [
                {
