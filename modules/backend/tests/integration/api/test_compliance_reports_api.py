"""
The compliance module's API — report generation and audit export.

Split out of ``test_compliance_api.py`` so that file tests only what a core
build serves (``GET /compliance/audit-events`` and the audit events
themselves) and the core build can delete this one.  The two routes below
keep their URLs in every profile but delegate their bodies through the seam;
without the modules' registration they answer 501.
"""
...
class TestComplianceReports:
    ...
    def test_soc2_report_returns_200(self, admin_client, db_session):
        ...
        with patch(
            "modules.backend.app.services.compliance_report_service.ComplianceReportService.generate_report",
...
class TestAuditExport:
    ...
    def test_admin_can_export_json(self, admin_client, db_session):
        ...
        with patch(
            "modules.backend.app.services.compliance_report_service.ComplianceReportService.export_events",
