...
class JiraService:
    ...
    def verify_webhook(
        ...
        """Authenticate an inbound Jira webhook against the shared secret.

        Jira Cloud signs the raw body with the webhook's secret and sends
        ``X-Hub-Signature: sha256=<hex>``; Jira Server has no secret support
        at all, so a relay in front of it presents the secret directly in
        ``X-Experimently-Webhook-Secret`` instead.  Both are checked in
        constant time; with no ``webhook_secret`` configured, neither passes.
        ...
        """
        return webhook_auth.verify_webhook(
            self._webhook_secret, payload_body, signature_header, secret_header
        )
