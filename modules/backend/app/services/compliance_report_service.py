...
# Verified through the seam, with whatever signer the modules' registration
...
class ComplianceReport:
    ...
    Attributes:
        ...
        unsigned_events: Events that carried no signature and so could not be
            checked at all. Unsigned rows are a reachable state -- the core
            signer writes none, and a process whose modules' registration
            failed falls back to it -- so a report that counted
