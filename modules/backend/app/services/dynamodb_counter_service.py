...
class DynamoDBCounterService:
    ...
    Args:
        table_name: DynamoDB table name.  Defaults to
                    ``ModulesSettings.DYNAMODB_COUNTERS_TABLE``, which is read
                    from the process environment and from the active
                    environment's dotenv file (``.env.dev``/``.env.prod``) --
                    the mechanism ``docs/getting-started/environment-setup.md``
                    documents.  Reading ``os.environ`` here directly, as this
                    did, meant a value set in ``.env.prod`` was loaded onto the
                    settings object and then ignored.
    ...
    def __init__(
        ...
        if table_name is None:
            # Imported here, not at module import: the settings are built and
            # validated by ``modules.register(hooks)``, and this module is
            # imported by the schema builders too.
            from modules.backend.app.settings import settings

            table_name = settings.DYNAMODB_COUNTERS_TABLE
