"""
Counter Integration for the real-time DynamoDB counters (P2-B).

Provides helpers that an event processor can call when it handles
...
Lives with the rest of the real-time counter cluster
(``dynamodb_counter_service``, ``schemas/realtime_counters``) rather than in
``backend/lambda/event_processor``: nothing in that Lambda package imports it,
and keeping it there made a core Lambda depend on an optional module.

Usage::

    from modules.backend.app.services.dynamodb_counter_integration import (
        process_assignment_event,
        process_conversion_event,
    )
...
from modules.backend.app.schemas.realtime_counters import CounterType
from modules.backend.app.services.dynamodb_counter_service import DynamoDBCounterService
